#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# This project forked from tzutalin/labelImg, New labelVideo.py for Video file labeling
# 
import codecs
...
class MainWindow(QMainWindow, WindowMixin):
	...
	def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
		...
		# Additional attributes for video process
		self.last_frame = 0
		self.cap = None
		self.file_type = 'image'
		self.videoTimer = QTimer()
		self.videoTimer.timeout.connect(self.video_render_frame)
		...
		# Additional video process actions
		play = action('&Play', self.play,
							'Ctrl+p', 'play', u'Start video play')

		stop = action('&Stop', self.stop,
							'Ctrl+Q', 'stop', u'Stop video play')

		next_frame = action('&Next Frame', self.next_frame,
							'Ctrl+.', 'next_frame', u'Next frame')

		prev_frame = action('&Previous Frame', self.prev_frame,
							'Ctrl+,', 'prev_frame', u'Previous frame')
		...
		# Create actions for media playback
		self.playbackActions = struct(play=play, stop=stop,
							fileMenuActions=(
		...
		addActions(self.menus.file,
				   (open, changeSavedir, openAnnotation, self.menus.recentFiles, save, save_format, saveAs, close, resetAll, quit))
		...
		# Create playback toolbar
		# Create toolbar widget for video play
		self.videoPlayback = ToolBar('Video Playback')
		self.videoPlayback.setObjectName(u'Playback')
		self.videoPlayback.setOrientation(Qt.Vertical)
		self.videoPlayback.setToolButtonStyle(Qt.ToolButtonIconOnly)
		self.videoPlayback.setFloatable(False)
		self.videoPlayback.setMovable(False)
		addActions(self.videoPlayback, (play, stop, prev_frame, next_frame))
		self.playbackSlider = QSlider()
		self.playbackSlider.setOrientation(Qt.Horizontal)
		self.playbackSlider.setFixedWidth(200)
		self.playbackSlider.sliderReleased.connect(self.on_video_tracked)
		self.videoPlayback.addWidget(self.playbackSlider)
		self.addToolBar(Qt.BottomToolBarArea, self.videoPlayback)

		# Toolbar
		self.tools = self.toolbar('Tools')
		self.actions.beginner = (
			open, changeSavedir, verify, save, save_format, None, create, copy, delete, None,
			zoomIn, zoom, zoomOut, fitWindow, fitWidth)

		self.actions.advanced = (
			open, changeSavedir, save, save_format, None,
	...
	def renderFrame(self, image):
		if image == None or image.isNull():
			self.errorMessage(u'Error render image',
	...
	# 
	# Additional video file process
	def loadVideo(self, filePath=None):
		"""Load the video file, or the last opened file if None."""
		self.resetState()
		...
		if unicodeFilePath and os.path.exists(unicodeFilePath):
			if LabelFile.isLabelFile(unicodeFilePath):
				...
				self.cap = cv2.VideoCapture(unicodeFilePath)
				self.video_file = unicodeFilePath
				self.last_frame = 0
				self.labelFile = None
				self.canvas.verified = False
				self.file_type = 'video'
				self.frame_rate = self.cap.get(cv2.CAP_PROP_FPS)
				self.videoTimer.setInterval(int(1000 / self.frame_rate))
				self.frame_count = self.cap.get(cv2.CAP_PROP_FRAME_COUNT)
				self.duration = self.frame_count / self.frame_rate	# seconds
				self.playbackSlider.setMinimum(1)
				self.playbackSlider.setMaximum(self.frame_count)

			image = self.read_frame()
			#ret, frame = self.cap.read()
			#frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
			#self.last_frame = self.cap.get(cv2.CAP_PROP_POS_FRAMES) - 1
			#image = QImage.fromData(self.imageData)
			#image = QImage(frame.data, frame.shape[1], frame.shape[0], QImage.Format_RGB888)
			imagefile, ext = os.path.splitext(filePath)
	...
	def read_frame(self):
		try:
			...
			frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
			self.last_frame = self.cap.get(cv2.CAP_PROP_POS_FRAMES) - 1
			path, ext = os.path.splitext(self.video_file)
			self.filePath = path + "." + str(int(self.last_frame)) + '.jpg'
			image = QImage(frame.data, frame.shape[1], frame.shape[0], QImage.Format_RGB888)
			return image
		except:
			return None

	#def video_playing_thread()
	def video_render_frame(self):
		if self.cap.isOpened():
			if self.last_frame >= self.frame_count - 1:
				self.videoTimer.stop()
				return
			image = self.read_frame()
			if image:
				imagefile, ext = os.path.splitext(self.filePath)
				self.renderFrame(image)
				self.playbackSlider.setValue(self.last_frame)
			else:
				self.videoTimer.stop()

	def on_video_tracked(self):
		self.cap.set(cv2.CAP_PROP_POS_FRAMES, self.playbackSlider.value() - 1)
		image = self.read_frame()
		imagefile, ext = os.path.splitext(self.filePath)
		self.renderFrame(image)
	...
	def pause(self, _value=False):
		self.video_playing = False
		self.videoTimer.stop()

	def stop(self, _value=False):
		self.video_playing = False
		self.videoTimer.stop()

	def next_frame(self, _value=False):
	...
	def prev_frame(self, _value=False):
		# Proceding prev image without dialog if having any label
		if self.autoSaving.isChecked():
			if self.defaultSaveDir is not None:
				if self.dirty is True:
		...
		self.cap.set(cv2.CAP_PROP_POS_FRAMES, self.last_frame - 1)
		image = self.read_frame()
		self.renderFrame(image)
	...
	def saveFile(self, _value=False):
		...
		if self.file_type == 'video':
			# Save current frame to jpeg image
			self.image.save(self.filePath, 'JPG')			
